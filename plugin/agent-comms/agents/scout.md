---
name: scout
description: "agent-comms 插件自带的 scout 只读核查类型：持有实时汇报协议，工具面按官方『侦察员=只读』原则配置（检索/读取/联网核实），不修改任何文件、不执行有副作用的命令。适合派去做资料查证、代码勘察、交叉核验等只读任务并实时回报。派发 prompt 里必须写「comms 频道」与「worker 名」两行。Read-only recon type with the same reporting protocol: search/read/verify and report in real time; never mutates files. Dispatch prompts must include the 'comms channel' and 'worker name' lines."
color: blue
